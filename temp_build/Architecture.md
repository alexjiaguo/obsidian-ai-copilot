...
### 2.3 AI Service Layer (`services/AIProvider.ts`)
- **Abstract Provider**: A base class/interface defining standard methods (`generateText`, `summarize`, `chat`).
- **Concrete Implementations**:
    - `OpenAIProvider.ts`
    - `AnthropicProvider.ts`
    - `LocalLLMProvider.ts` (Ollama/LocalAI)

### 2.4 Editor Integration (`EditorHandler.ts`)
...
### 2.5 AI Sidebar Chat (`views/AIChatView.ts`)
...
### 2.6 Vault Indexer & RAG (`services/VectorIndex.ts`)
...
│   ├── services/          # AI providers and Vector Index
